...
// Номер/юзернейм менеджера для оформлення замовлення через Telegram (ручний варіант — відкриває чат).
const TELEGRAM_MANAGER = "smokershop";

// --- АВТОМАТИЧНА ВІДПРАВКА В TELEGRAM-КАНАЛ ---
// 1. Створи бота через @BotFather -> /newbot -> скопіюй токен сюди.
// 2. Додай бота адміністратором у свій канал.
// 3. Впиши юзернейм каналу (з "@" на початку), якщо канал публічний.
// ⚠️ Токен буде видно у вихідному коді сайту (це статичний сайт без бекенду).
//    Якщо сайт публічний і це критично — краще винести відправку на простий
//    сервер/serverless-функцію. Для приватного використання так теж працює.
const TELEGRAM_BOT_TOKEN = "ВАШ_ТОКЕН_ТУТ";      // напр. "123456789:AAHdq..."
const TELEGRAM_CHANNEL = "@ваш_канал";            // напр. "@smokershop_orders"

async function sendToTelegramChannel(text) {
    if (!TELEGRAM_BOT_TOKEN || TELEGRAM_BOT_TOKEN === "ВАШ_ТОКЕН_ТУТ") {
        showToast('Бот не налаштований (вкажіть токен у script.js)');
        return false;
    }
    try {
        const res = await fetch(`https://t.me/+XlM7Nj8wJv1kNWVi`, {
            method: 'POST',
...
async function checkout() {
    ...
    if (sent) {
        ...
        // Бот не налаштований або сталась помилка — відкриваємо ручний варіант
        const link = 'https://t.me/+XlM7Nj8wJv1kNWVi}';
