...
// Номер/юзернейм менеджера для оформлення замовлення через Telegram.
// ⚠️ ЗАМІНІТЬ на реальний юзернейм вашого магазину перед публікацією!
...
function checkout() {
    ...
    let msg = "Замовлення з сайту SmokerShop:\n";
    let total = 0;
    cart.forEach(item => {
        const product = findProduct(item.id);
        if (!product) return;
